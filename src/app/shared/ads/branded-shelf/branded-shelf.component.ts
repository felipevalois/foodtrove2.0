...
@Component({
  selector: 'app-branded-shelf',
  imports: [CommonModule, ProductCardComponent, MatTooltipModule],
  templateUrl: './branded-shelf.component.html',
  styleUrl: './branded-shelf.component.scss'
})

export class BrandedShelfComponent {
  @Input({ required: true }) data!: BrandStripData;
  /** how many cards are fully visible ( matches FoodTrove carousel = 4 ) */
  readonly cardOnly  = 178;    // real card width
  readonly gap       = 20;     // 1.25 rem you use in CSS
  readonly visible   = 3;      // how many cards you want to see
